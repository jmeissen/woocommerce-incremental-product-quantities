...
if ( ! class_exists( 'IPQ_Quantity_Rule_Post_Type' ) ) :
	...
	class IPQ_Quantity_Rule_Post_Type {
		 ...
		 *
		 *  @param array $columns Rule columns;
		 *  @return array
		 */
		...
		public function print_tax_inputs( $term, $taxonomy_name, $cats, $level ) {
			...
			?>
			<li>
				<input type="checkbox" id="_wpbo_cat_<?php echo $term->term_id; ?>" name="_wpbo_cat_<?php echo $term->term_id; ?>"
																<?php
																if ( is_array( $cats ) and in_array( $term->term_id, $cats ) ) {
		...
		*/
			...
			if ( $tags ) {
				...
				<ul id='product-tags' class='rule-product-cats'>
					<?php foreach ( $tags as $tag ) : ?>
						<li>
							<input type='checkbox' name='_wpbo_tag_<?php echo $tag->term_id; ?>' id='wpbo_tag_<?php echo $tag->term_id; ?>'
																			  <?php
																				if ( in_array( $tag->term_id, $included_tags ) ) {
																					echo 'checked';}
																				?>
							 />
							<?php echo $tag->name; ?>
		...
		public function quantity_rule_role( $post ) {
			...
			<ul>
					...
					<li>
						<input type='checkbox' name='_wpbo_role_<?php echo $slug; ?>' id='wpbo_role_<?php echo $slug; ?>'
																		   <?php
																			if ( in_array( $slug, $applied_roles ) || empty( $applied_roles ) ) {
																				echo 'checked';}
																			?>
						 />
						<?php echo $name; ?>
				...
				<?php endforeach; ?>
				<li>
					<input type='checkbox' name='_wpbo_role_guest' id='wpbo_role_guest'
					<?php
					if ( in_array( 'guest', $applied_roles ) || empty( $applied_roles ) ) {
						echo 'checked';}
					?>
					 />
				Guest
